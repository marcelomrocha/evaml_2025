...
#fotofundo
back = Label(janela)
back.la = PhotoImage(file = 'sim_tts_piper/images/tts_piper.png')
back['image'] = back.la
back.place(x=0,y=0)
...
def on_message(client, userdata, msg):
    ...
    if not (os.path.isfile(file_path + file_name + config.WATSON_AUDIO_EXTENSION)): # If it doesn't exist, call Piper.
        ...
        audio =  AudioSegment.from_file("sim_tts_piper/temporary.mp3")
        ...
        sf.write("sim_tts_piper/tts_cache_files/" + file_name + config.WATSON_AUDIO_EXTENSION, y_pitched, sr)  # WAV é melhor que MP3

    client.publish(topic_base + "/expression", "speech_on_1")
